...
describe("connection-gater with arrow function properties", () => {
  ...
  it("should deny dialling a peer", async () => {
    ...
    await expect(localNode.dial(ma)).to.eventually.be.rejected.with.property(
      "name",
      "DialDeniedError"
    );

    expect(connectionGater.denyDialPeer?.called).to.be.true();
    expect(connectionGater.denyDialPeer?.getCall(0).args[0]).to.deep.equal(
      remoteNode.peerId
    );
  });
  ...
  it("should deny dialling a multiaddr", async () => {
    ...
    await expect(
      localNode.dial(remoteNode.getMultiaddrs())
  ...
  it("should deny an inbound connection", async () => {
    ...
    ).to.eventually.be.rejected.with.property("name", "EncryptionFailedError");
  ...
  it("should deny an outbound connection", async () => {
    ...
    ).to.eventually.be.rejected.with.property(
      "name",
  ...
  it("should deny an inbound encrypted connection", async () => {
    ...
    await expect(
      remoteNode.dial(localNode.getMultiaddrs())
    ).to.eventually.be.rejected.with.property("name", "MuxerUnavailableError");
  ...
  it("should deny an outbound encrypted connection", async () => {
    ...
    expect(
      connectionGater.denyOutboundEncryptedConnection?.called
    ).to.be.true();
  });
...
describe("connection-gater with class methods", () => {
  ...
  it("should deny dialling a multiaddr", async () => {
    ...
    const connectionGater = new TestGater();
    const denyDialMultiaddrStub = Sinon.spy(
      connectionGater,
      "denyDialMultiaddr"
    );
  ...
  it("should deny an inbound connection", async () => {
    ...
    const connectionGater = new TestGater();
    const denyInboundConnectionStub = Sinon.spy(
      connectionGater,
  ...
  it("should deny an outbound connection", async () => {
    ...
    const connectionGater = new TestGater();
    const denyOutboundConnectionStub = Sinon.spy(
      connectionGater,
  ...
  it("should deny an inbound encrypted connection", async () => {
    ...
    const connectionGater = new TestGater();
    const denyInboundEncryptedConnectionStub = Sinon.spy(
      connectionGater,
  ...
  it("should deny an outbound encrypted connection", async () => {
    ...
    const connectionGater = new TestGater();
    const denyOutboundEncryptedConnectionStub = Sinon.spy(
      connectionGater,
  ...
  it("should deny an inbound upgraded connection", async () => {
    ...
    const connectionGater = new TestGater();
    const denyInboundUpgradedConnectionStub = Sinon.spy(
      connectionGater,
  ...
  it("should deny an outbound upgraded connection", async () => {
    ...
    const connectionGater = new TestGater();
    const denyOutboundUpgradedConnectionStub = Sinon.spy(
      connectionGater,
